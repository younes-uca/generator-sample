...
public async archiverCommandeTag(commandeTag:CommandeTagVo){
const isPermistted = await this.roleService.isPermitted('CommandeTag', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous archiver cet élément (Commande tag) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.commandeTagService.archiver(commandeTag).subscribe(status=>{
const myIndex = this.commandeTags.indexOf(commandeTag);
this.commandeTags[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Commande tag archivé',
life: 3000
});
},error=>console.log(error))
 }
...
public async desarchiverCommandeTag(commandeTag:CommandeTagVo){
const isPermistted = await this.roleService.isPermitted('CommandeTag', 'delete');
if(isPermistted){
this.confirmationService.confirm({
message: 'Voulez-vous désarchiver cet élément (Commande tag) ?',
header: 'Confirmation',
icon: 'pi pi-exclamation-triangle',
accept: () => {
this.commandeTagService.desarchiver(commandeTag).subscribe(status=>{
const myIndex = this.commandeTags.indexOf(commandeTag);
this.commandeTags[myIndex] = status;
this.messageService.add({
severity: 'success',
summary: 'Succès',
detail: 'Commande tag désarchivé',
life: 3000
});
},error=>console.log(error))
 }

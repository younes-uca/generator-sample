...
public class CommandeTagAdminServiceImpl extends AbstractServiceImpl<CommandeTag> implements CommandeTagAdminService {
    ...
    @Override
    public CommandeTag archiver(CommandeTag commandeTag) {
        if (commandeTag.getArchive() == null) {
            commandeTag.setArchive(false);
        }
        commandeTag.setArchive(true);
        commandeTag.setDateArchivage(new Date());
        commandeTagDao.save(commandeTag);
        return commandeTag;

    }

    @Override
    public CommandeTag desarchiver(CommandeTag commandeTag) {
        if (commandeTag.getArchive() == null) {
            commandeTag.setArchive(false);
        }
        commandeTag.setArchive(false);
        commandeTag.setDateArchivage(null);
        commandeTagDao.save(commandeTag);
        return commandeTag;
    }
